    ...
    private
      ...
      def handle_example(opts)
        ...
        if example
          ...
          example_class = opts.delete('example_class') || 'example ' + css_class
          real_text_class = opts.delete('real_text_class') || 'real-text ' + css_class
          js = "addExampleText("
          js << "$('#{escape_javascript(opts["id"])}'),"
          js << "'#{escape_javascript(example)}',"
          js << "'#{escape_javascript(example_class)}',"
          js << "'#{escape_javascript(real_text_class)}')"
          javascript_tag(js)
        else
          ''
        end
